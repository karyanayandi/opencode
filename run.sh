...
# Install as opencode
install_opencode() {
  print_header "🔗 INSTALL AS opencode"

  echo "This installs the patched binary as 'opencode' in ~/.local/bin"
  ...
  *)
  ...
  print_success "Copied to ~/.local/bin/opencode"
  echo ""

  # Verify
  if command -v opencode &>/dev/null; then
    echo "  opencode (global): $(opencode --version 2>/dev/null || echo 'unknown')"
    echo "  opencode    (patched): $(opencode --version 2>/dev/null || echo 'unknown')"
  else
    print_warning "~/.local/bin is not in your PATH"
    echo ""
    echo "Add to your shell config (~/.bashrc or ~/.zshrc):"
...
clear_cache() {
  ...
  1)
    ...
    if [ -d "$plugin_dir" ]; then
      ...
      echo "  Plugins will be re-downloaded fresh on next opencode run."
    else
  ...
  2)
    ...
    echo "  Everything will be re-downloaded fresh on next opencode run."
    ;;
  3)
    echo "Cancelled."
    ;;
...
build_opencode() {
  ...
  if [ -f "$SCRIPT_DIR/build.sh" ]; then
    ...
    echo ""
    read -p "Install as opencode now? (y/N): " install
    if [[ $install =~ ^[Yy]$ ]]; then
      install_opencode
    fi
  else
...
show_menu() {
  ...
  echo "    6. 📥 Install as opencode                       - Copy to ~/.local/bin/opencode"
...
main() {
  ...
  if [ $# -gt 0 ]; then
    ...
    "help" | "--help" | "-h")
      ...
      echo "  install-opencode    - Copy patched binary to ~/.local/bin/opencode"
